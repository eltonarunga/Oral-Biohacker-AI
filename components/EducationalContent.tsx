...
const ArticleCard: React.FC<{ article: EducationalArticle }> = ({ article }) => {
  return (
    <div className="bg-input-light dark:bg-input-dark border border-subtle-light/20 dark:border-subtle-dark/20 rounded-xl shadow-sm p-4">
      <div className="flex items-start gap-4">
        <div className="flex-shrink-0 size-10 bg-primary/10 text-primary flex items-center justify-center rounded-lg">
            <span className="material-symbols-outlined">{article.icon}</span>
        </div>
        <div className="flex-1">
            <h3 className="font-bold text-base text-foreground-light dark:text-foreground-dark">{article.title}</h3>
            <p className="text-xs text-subtle-light dark:text-subtle-dark mb-2">Source: {article.source}</p>
            <p className="text-sm text-subtle-light dark:text-subtle-dark mb-3">{article.summary}</p>
            <a href={article.link} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline font-semibold flex items-center gap-1">
...
const FormattedContent: React.FC<{ text: string }> = ({ text }) => {
    ...
    return (
        <div className="text-subtle-light dark:text-subtle-dark space-y-2 prose dark:prose-invert prose-sm max-w-none">
            {text.split('\n').map((line, i) => {
...
const EducationalContent: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                    <input
                        ...
                        disabled={isLoading}
                        className="flex-1 w-full bg-background-light dark:bg-input-dark border border-subtle-light/20 dark:border-subtle-dark/20 rounded-lg p-3 text-foreground-light dark:text-foreground-dark focus:ring-primary focus:border-primary disabled:opacity-50"
                        aria-label="Ask a question about oral health"
                    />
                    <button 
                        onClick={handleGenerate} 
                        disabled={isLoading || !query.trim()}
                        className="bg-primary hover:opacity-90 text-white font-bold py-3 px-6 rounded-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                ...
                {result && (
                    <div className="pt-4 border-t border-subtle-light/20 dark:border-subtle-dark/20">
                        <div className="flex justify-between items-start mb-2">
                           <h3 className="text-lg font-bold text-foreground-light dark:text-foreground-dark">AI-Generated Answer</h3>
                           <TTSButton textToSpeak={result.text} className="text-primary hover:bg-primary/10" />
                        </div>
                        <FormattedContent text={result.text} />
                        
                        {result.sources.length > 0 && (
                             <div className="mt-4">
                                <h4 className="text-xs font-bold text-subtle-light dark:text-subtle-dark mb-1.5 uppercase tracking-wider">Sources</h4>
        ...
        <div>
            <h2 className="text-2xl font-bold text-foreground-light dark:text-foreground-dark">Oral Health Knowledge Base</h2>
            <p className="text-base text-subtle-light dark:text-subtle-dark mt-1">
