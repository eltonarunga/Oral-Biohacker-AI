...
const SupplementIcon = () => (<svg xmlns="http://www.w.org/2000/svg" className="h-5 w-5 mr-2 inline" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5 3a1 1 0 000 2c.552 0 1 .448 1 1v1a1 1 0 001 1h6a1 1 0 001-1V6c0-.552.448-1 1-1a1 1 0 100-2H5z" clipRule="evenodd" /><path d="M4 9a1 1 0 011-1h10a1 1 0 011 1v5a1 1 0 01-1 1H5a1 1 0 01-1-1V9z" /></svg>);
const RoutineIcon = () => (<svg xmlns="http://www.w.org/2000/svg" className="h-5 w-5 mr-2 inline" viewBox="0 0 20 20" fill="currentColor"><path d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" /></svg>);
    ...
    <Card title="Your AI-Generated Biohacking Plan" icon={<PlanIcon />}>
      ...
      {!isLoading && !plan && (
        <div className="text-center">
            <p className="text-gray-500 mb-4">Your personalized oral health plan will appear here. Update your biometrics and generate a plan tailored just for you.</p>
      ...
      {plan && (
        <div className="space-y-6">
          <section>
            <h3 className="text-lg font-semibold mb-2 text-blue-600 flex items-center"><AlertIcon />Key Alerts & Markers</h3>
            <ul className="space-y-2">
                {plan.alerts.map((alert, index) => (
                    <li key={index} className="bg-white p-3 rounded-lg flex justify-between items-center border border-gray-200">
                        <div>
                            <p className="font-semibold text-gray-800">{alert.marker}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <section>
                <h3 className="text-lg font-semibold mb-2 text-blue-600 flex items-center"><SupplementIcon />Supplement Protocol</h3>
                <ul className="space-y-2 text-sm">
                    {plan.supplements.map((s, i) => <li key={i} className="bg-white p-3 rounded-lg border border-gray-200"><strong className="text-gray-900">{s.name}</strong> ({s.dosage}): <span className="text-gray-600">{s.reason}</span></li>)}
                </ul>
            </section>
            <section>
                <h3 className="text-lg font-semibold mb-2 text-blue-600 flex items-center"><RoutineIcon />Habits & Routines</h3>
                 <ul className="space-y-2 text-sm">
                    {plan.routines.map((r, i) => <li key={i} className="bg-white p-3 rounded-lg border border-gray-200"><strong className="text-gray-900">{r.name}</strong> ({r.frequency}): <span className="text-gray-600">{r.instructions}</span></li>)}
                </ul>
            </section>
          </div>
          <section>
             <h3 className="text-lg font-semibold mb-2 text-blue-600 flex items-center"><NutritionIcon />Nutritional Guidance</h3>
             <ul className="space-y-2 text-sm">
                {plan.nutrition.map((n, i) => <li key={i} className="bg-white p-3 rounded-lg border border-gray-200"><strong className="text-gray-900">{n.recommendation}</strong>: <span className="text-gray-600">{n.reason}</span></li>)}
            </ul>
          </section>
