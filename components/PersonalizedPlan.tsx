    ...
    <Card title="Your AI-Generated Biohacking Plan" icon={<PlanIcon />}>
      ...
      {!isLoading && !plan && (
        <div className="text-center">
            <p className="text-slate-400 mb-4">Your personalized oral health plan will appear here. Update your biometrics and generate a plan tailored just for you.</p>
      ...
      {plan && (
        <div className="space-y-6">
          <section>
            <h3 className="text-lg font-semibold mb-2 text-cyan-300 flex items-center"><AlertIcon />Key Alerts & Markers</h3>
            <ul className="space-y-2">
                {plan.alerts.map((alert, index) => (
                    <li key={index} className="bg-slate-700/50 p-3 rounded-lg flex justify-between items-center">
                        <div>
                            <p className="font-semibold text-white">{alert.marker}</p>
                            <p className="text-sm text-slate-300">{alert.advice}</p>
                        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <section>
                <h3 className="text-lg font-semibold mb-2 text-cyan-300 flex items-center"><SupplementIcon />Supplement Protocol</h3>
                <ul className="space-y-2 text-sm">
                    {plan.supplements.map((s, i) => <li key={i} className="bg-slate-700/50 p-3 rounded-lg"><strong className="text-white">{s.name}</strong> ({s.dosage}): <span className="text-slate-300">{s.reason}</span></li>)}
                </ul>
            </section>
            <section>
                <h3 className="text-lg font-semibold mb-2 text-cyan-300 flex items-center"><RoutineIcon />Habits & Routines</h3>
                 <ul className="space-y-2 text-sm">
                    {plan.routines.map((r, i) => <li key={i} className="bg-slate-700/50 p-3 rounded-lg"><strong className="text-white">{r.name}</strong> ({r.frequency}): <span className="text-slate-300">{r.instructions}</span></li>)}
                </ul>
            </section>
          </div>
          <section>
             <h3 className="text-lg font-semibold mb-2 text-cyan-300 flex items-center"><NutritionIcon />Nutritional Guidance</h3>
             <ul className="space-y-2 text-sm">
                {plan.nutrition.map((n, i) => <li key={i} className="bg-slate-700/50 p-3 rounded-lg"><strong className="text-white">{n.recommendation}</strong>: <span className="text-slate-300">{n.reason}</span></li>)}
            </ul>
          </section>
          <button onClick={onGeneratePlan} className="w-full mt-4 bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded-lg transition duration-200">
            Regenerate Plan
          </button>
        </div>
      )}
